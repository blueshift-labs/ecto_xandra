if Code.ensure_loaded?(Xandra) do
  defmodule EctoXandra.Connection do
    ...
    # alias Ecto.Adapters.Xandra.RetryStrategy
    alias Xandra.{Prepared}

    @behaviour Ecto.Adapters.SQL.Connection

    @conn EctoXandra.Conn

    ## Connection

    @impl true
    def child_spec(opts) do
      opts = Keyword.put_new(opts, :name, @conn)
      opts = Keyword.put(opts, :uuid_format, :binary)
      opts = Keyword.put(opts, :atom_keys, true)
      opts = 
        case Keyword.get(opts, :keyspace, nil) do
          nil -> opts
          keyspace ->
            Keyword.put(opts, :after_connect, &Xandra.execute!(&1, "USE #{keyspace}"))
        end
      Supervisor.child_spec({Xandra.Cluster, opts}, id: Keyword.fetch!(opts, :name))
    end
    ...
    defp process_page(%Xandra.Page{content: content} = page) do
    ...
    def execute(cluster, query, params, opts) do
      require IEx; IEx.pry
      Xandra.Cluster.execute(cluster, query, params, opts)
    end
    ...
    def all(query, as_prefix \\ []) do
      ...
      stmt = [select, from, where, order_by, limit]
    ...
    @impl true
    def insert(prefix, table, headers, rows, on_conflict, returning, opts) when prefix in [nil, ""] do
    ...
    defp insert_suffix(opts) do
      suffix = case Keyword.get(opts, :overwrite, true) do
        true -> []
        _ -> [" IF NOT EXISTS"]
      end

      suffix = case Keyword.get(opts, :ttl, nil) do
        nil-> suffix
        seconds when is_number(seconds)-> suffix ++ [" USING TTL #{seconds}"]
      end

      suffix = case Keyword.get(opts, :timestamp, nil) do
        nil -> 
          suffix
    ...
    # defp format_table(nil, table), do: table
    # defp format_table("", table), do: table
    # defp format_table(prefix, table), do: "#{prefix}.#{table}"
    ...
    def update(prefix, table, fields, filters, _returning) do
      # keys = headers |> Enum.join(", ")
      # values = rows |> Enum.map(&("("<> Enum.map_join(&1, ", ", fn _ -> "?" end)<>")"))
      # "UPDATE #{format_table(prefix, table)} (#{keys}) VALUES #{values} #{insert_suffix(opts)}"
      ...
      intersperse_map(fields, ", ", fn
        {:&, _, [idx]} ->
          case elem(sources, idx) do
            {source, _, nil} ->
              error!(query, "Cassandra does not support selecting all fields from #{source} without a schema. " <>
    ...
    defp expr({{:., _, [{:&, _, [_idx]}, field]}, _, []}, sources, _query)
        when is_atom(field) do
      [quote_name(field)]
